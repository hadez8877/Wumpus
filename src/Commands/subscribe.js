...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("language")
        .setDescription("Escoge el lenguaje en el que responda el bot")
        .addChoices(
  ...
  async execute(interaction) {
    ...
    if (data) {
      ...
      const languageReplace = language.replace("es", "Español").replace("en", "English").replace("pt", "Português").replace("de", "Deutsch").replace("it", "Italiano").replace("ja", "日本語").replace("zh", "中文").replace("ko", "한국어").replace("ru", "Русский").replace("ar", "العربية");
      ...
      embed = new EmbedBuilder()
        ...
        .addFields(
          { name: "\u200B", value: "\u200B" },
          {
            name: `🗣️ ${await sendTranslated("Lenguaje:", interaction.guild.id)}`,
            value: `\`\`${languageReplace}\`\``,
            inline: true
          },
          {
            name: `💾 ${await sendTranslated("Prefijo:", interaction.guild.id)}`,
            value: `\`\`${prefix ?? "s!"}\`\``,
            inline: true
          },
          {
            name: `📣 ${await sendTranslated("Notificaciones:", interaction.guild.id)}`,
            value: `\`\`${notificationsReplace}\`\``,
            inline: true
          }
        )
      ...
      embed = new EmbedBuilder()
        ...
        .setDescription(
          `${await sendTranslated(`La configuración del bot se ha completado con éxito.`, interaction.guild.id)} ${await sendTranslated(`Ahora está listo para ser utilizado en tu servidor.`, interaction.guild.id)} ${await sendTranslated(`¡Disfruta explorando todas las funciones y posibilidades que ofrece!`, interaction.guild.id)}`
        )
        .addFields(
          { name: "\u200B", value: "\u200B" },
          {
